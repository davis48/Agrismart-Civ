...
exports.register = async (req, res, next) => {
  try {
    const { email, telephone, password, nom, prenoms, prenom, langue_preferee = 'fr' } = req.body;
    ...
    // En mode développement, activer directement le compte
    const initialStatus = (config.isDev || config.env === 'development' || process.env.NODE_ENV !== 'production') 
      ? 'actif' 
      : 'en_attente';
    ...
    // En mode développement, connexion directe
    if (config.isDev || config.env === 'development' || process.env.NODE_ENV !== 'production') {
      const accessToken = generateAccessToken(user);
      const refreshToken = await generateRefreshToken(user.id);

      logger.audit('Inscription et connexion directe (dev)', { userId: user.id, email });
...
exports.login = async (req, res, next) => {
  try {
    ...
    // En mode développement, connexion directe sans OTP
    if (config.isDev || config.env === 'development' || process.env.NODE_ENV !== 'production') {
      // Générer les tokens
      const accessToken = generateAccessToken(user);
      const refreshToken = await generateRefreshToken(user.id);
      ...
      logger.audit('Connexion directe (dev)', { userId: user.id, email: user.email });
