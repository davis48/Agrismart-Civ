...
const generateMockData = (days: number) => {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    data.push({
      ...
      dateComplete: date.toISOString(),
      temperature: Math.round((25 + Math.random() * 10 - 5) * 10) / 10,
      humidite_sol: Math.round((60 + Math.random() * 30 - 15) * 10) / 10,
      humidite_air: Math.round((70 + Math.random() * 20 - 10) * 10) / 10,
      luminosite: Math.round(40000 + Math.random() * 30000 - 15000),
      ph: Math.round((6.5 + Math.random() * 1 - 0.5) * 10) / 10,
...
const hourlyData = Array.from({ length: 24 }, (_, i) => ({
  heure: `${i.toString().padStart(2, '0')}h`,
  temperature: Math.round((20 + Math.sin(i / 3.8) * 8 + Math.random() * 2) * 10) / 10,
  humidite_sol: Math.round((55 + Math.cos(i / 3.8) * 15 + Math.random() * 5) * 10) / 10,
  luminosite: i >= 6 && i <= 18 
    ? Math.round(10000 + Math.sin((i - 6) / 3.8) * 50000 + Math.random() * 5000)
...
export default function MesuresPage() {
  ...
  const [chartData, setChartData] = useState<typeof hourlyData | ReturnType<typeof generateMockData>>(generateMockData(7))
  ...
  const calculateStats = (): StatCard[] => {
    ...
    return [
      ...
      {
        label: 'pH moyen',
        value: 'ph' in chartData[0] 
          ? (chartData.reduce((acc, d) => acc + ('ph' in d ? d.ph : 0), 0) / chartData.length).toFixed(1)
          : '6.5',
        unit: 'pH',
        trend: 'stable',
        change: '±0.1',
              ...
              <AreaChart data={chartData}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [
                    `${value} ${getMetricConfig(activeMetric).unit}`,
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value: number) => [`${value} mm`, 'Précipitations']}
                    />
                    <Bar
